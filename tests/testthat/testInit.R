...
#   opts2 <- antaresRead::setSimulationPath(testStudy2)
#   
#   fb_opts <- system.file("testdata/antaresInput", package = "fbAntares")
#   if(fb_opts == "") fb_opts <- system.file("testdata/antaresInput", package = "fbAntares")
#   # init <- try(suppressWarnings(initFlowBased(fb_opts = fb_opts, 
#   #                                            scenarios = rep(1:200, times = 5), opts = opts2)))
